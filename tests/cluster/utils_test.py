...
from yaaredis.utils import blocked_command
from yaaredis.utils import clusterdown_wrapper
from yaaredis.utils import dict_merge
from yaaredis.utils import first_key
from yaaredis.utils import list_keys_to_dict
...
def test_parse_cluster_slots():
    ...
    extended_mock_response = [
        [0, 5460, ['172.17.0.2', 7000, 'ffd36d8d7cb10d813f81f9662a835f6beea72677'], [
            '172.17.0.2', 7003, '5c15b69186017ddc25ebfac81e74694fc0c1a160']],
        [5461, 10922, ['172.17.0.2', 7001, '069cda388c7c41c62abe892d9e0a2d55fbf5ffd5'], [
            '172.17.0.2', 7004, 'dc152a08b4cf1f2a0baf775fb86ad0938cb907dc']],
        [10923, 16383, ['172.17.0.2', 7002, '3588b4cf9fc72d57bb262a024747797ead0cf7ea'], [
    ...
    extended_mock_binary_response = [
        [0, 5460, [b('172.17.0.2'), 7000, b('ffd36d8d7cb10d813f81f9662a835f6beea72677')], [
            b('172.17.0.2'), 7003, b('5c15b69186017ddc25ebfac81e74694fc0c1a160')]],
        [5461, 10922, [b('172.17.0.2'), 7001, b('069cda388c7c41c62abe892d9e0a2d55fbf5ffd5')], [
            b('172.17.0.2'), 7004, b('dc152a08b4cf1f2a0baf775fb86ad0938cb907dc')]],
        [10923, 16383, [b('172.17.0.2'), 7002, b('3588b4cf9fc72d57bb262a024747797ead0cf7ea')], [
...
def test_dict_merge():
    ...
    y = {'b': 2}
    z = {'c': 3}
    assert dict_merge(x, y, z) == {'a': 1, 'b': 2, 'c': 3}
...
async def test_clusterdown_wrapper():
    ...
    with pytest.raises(ClusterDownError) as cex:
        await bad_func()
    assert str(cex.value).startswith(
