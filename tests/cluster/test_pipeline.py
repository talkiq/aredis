...
#                 pipe.execute()
#
#             assert ex.value.startswith('Command # 1 (LLEN a) of '
#                                                 'pipeline caused error: ')
#
#         assert r['a'] == b('1')
#
...
#                 pipe.execute()
#
#             expected = str('Command # 1 (LLEN {0}) of pipeline caused error: ').format(key)
#             assert ex.value.startswith(expected)
#
#         assert r[key] == b('1')
#
...
#
#         assert ex.value.startswith("transaction is deprecated in cluster mode"), True
#
#         with pytest.raises(RedisClusterException) as ex:
#             r.pipeline(shard_hint=True)
#
#         assert ex.value.startswith("shard_hint is deprecated in cluster mode"), True
#
