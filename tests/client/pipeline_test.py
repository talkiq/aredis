...
async def test_pipeline(r):
    ...
    async with await r.pipeline() as pipe:
        ...
        expected = [True, b('a1'), 1, 1, 2.0, [(b('z1'), 2.0), (b('z2'), 4)]]
        assert await pipe.execute() == expected


@pytest.mark.asyncio(forbid_global_loop=True)
async def test_pipeline_length(r):
    await r.flushdb()
...
@pytest.mark.asyncio(forbid_global_loop=True)
async def test_pipeline_autoexecute(r):
    await r.flushdb()
...
@pytest.mark.asyncio(forbid_global_loop=True)
async def test_pipeline_no_transaction(r):
    await r.flushdb()
...
@pytest.mark.asyncio(forbid_global_loop=True)
async def test_pipeline_no_transaction_watch(r):
    await r.flushdb()
    await r.set('a', 0)
...
@pytest.mark.asyncio(forbid_global_loop=True)
async def test_pipeline_no_transaction_watch_failure(r):
    await r.flushdb()
    await r.set('a', 0)
...
@pytest.mark.asyncio(forbid_global_loop=True)
async def test_exec_error_in_response(r):
    """
...
@pytest.mark.asyncio(forbid_global_loop=True)
async def test_exec_error_raised(r):
    await r.flushdb()
    await r.set('c', 'a')
...
@pytest.mark.asyncio(forbid_global_loop=True)
async def test_parse_error_raised(r):
    await r.flushdb()
...
@pytest.mark.asyncio(forbid_global_loop=True)
async def test_watch_succeed(r):
    await r.flushdb()
    await r.set('a', 1)
    await r.set('b', 2)
...
@pytest.mark.asyncio(forbid_global_loop=True)
async def test_watch_failure(r):
    await r.flushdb()
    await r.set('a', 1)
    await r.set('b', 2)
...
@pytest.mark.asyncio(forbid_global_loop=True)
async def test_transaction_callable(r):
    await r.flushdb()
    await r.set('a', 1)
    await r.set('b', 2)
    has_run = []
    ...
    result = await r.transaction(my_transaction, 'a', 'b', watch_delay=0.01)
    assert result == [True]
    assert await r.get('c') == b('4')


@pytest.mark.asyncio(forbid_global_loop=True)
async def test_exec_error_in_no_transaction_pipeline(r):
    await r.flushdb()
    await r.set('a', 1)
...
@pytest.mark.asyncio(forbid_global_loop=True)
async def test_exec_error_in_no_transaction_pipeline_unicode_command(r):
