...
async def test_xadd_with_wrong_id(r):
...
@skip_if_server_version_lt('4.9.103')
@pytest.mark.asyncio(forbid_global_loop=True)
async def test_xadd_without_given_id(r):
    await r.flushdb()
...
@skip_if_server_version_lt('4.9.103')
@pytest.mark.asyncio(forbid_global_loop=True)
async def test_xadd_with_given_id(r):
    await r.flushdb()
...
@skip_if_server_version_lt('4.9.103')
@pytest.mark.asyncio(forbid_global_loop=True)
async def test_xadd_with_maxlen_accurately(r):
    await r.flushdb()

    for _ in range(10):
        await r.xadd('test_stream', {'k1': 'v1', 'k2': 1}, max_len=2,
                     approximate=False)
    # also test xlen here
...
@skip_if_server_version_lt('4.9.103')
@pytest.mark.asyncio(forbid_global_loop=True)
async def test_xadd_with_maxlen_approximately(r):
    await r.flushdb()
...
async def test_xrevrange(r):
    ...
    assert len(entries) == 2
    assert entries[0][0] == b'3-0'


@skip_if_server_version_lt('4.9.103')
@pytest.mark.asyncio(forbid_global_loop=True)
async def test_xread(r):
    await r.flushdb()
    ...
    assert entries
    assert len(entries[b'test_stream']) == 7
    assert entries[b'test_stream'][0] == (b'3-0', {b'k1': b'v1', b'k2': b'1'})


@skip_if_server_version_lt('4.9.103')
@pytest.mark.asyncio(forbid_global_loop=True)
async def test_xreadgroup(r):
    await r.flushdb()
    ...
    with pytest.raises(ResponseError):
        await r.xreadgroup('wrong_group', 'lalala', count=10, block=10,
                           test_stream='1')

    assert await r.xgroup_create('test_stream', 'test_group', '0') is True

    entries = await r.xreadgroup('test_group', 'consumer1', count=5,
                                 test_stream='>')
    assert len(entries[b'test_stream']) == 5
...
@skip_if_server_version_lt('4.9.103')
@pytest.mark.asyncio(forbid_global_loop=True)
async def test_xgroup_create(r):
    await r.flushdb()
...
@skip_if_server_version_lt('4.9.103')
@pytest.mark.asyncio(forbid_global_loop=True)
async def test_xgroup_set_id(r):
    await r.flushdb()
...
@skip_if_server_version_lt('4.9.103')
@pytest.mark.asyncio(forbid_global_loop=True)
async def test_xgroup_destroy(r):
    await r.flushdb()
...
@skip_if_server_version_lt('4.9.103')
@pytest.mark.asyncio(forbid_global_loop=True)
async def test_xgroup_del_consumer(r):
    await r.flushdb()
...
async def test_xpending(r):
    ...
    pending = await r.xpending('test_stream', 'test_group', count=10,
                               consumer='consumer1')
    ...
    xpending_entries_in_range = await r.xpending('test_stream', 'test_group',
                                                 start='2', end='2', count=10,
                                                 consumer='consumer1')
